...
st.markdown("""
    ...
    .opp-theme label p { color: #991b1b !important; font-weight: bold !important; }
    ...
    if st.button("🌐 スプレッドシートに送信", use_container_width=True, type="primary"):
        if not GSHEETS_READY:
            ...
            try:
                ...
                df_send = pd.DataFrame(st.session_state.logs).drop(columns=['id'], errors='ignore')
                conn.update(data=df_send)
                st.success("送信完了！"); st.balloons()
...
st.markdown(f"<style>div.stButton > button {{ background-color: {btn_color} !important; color: white !important; }}</style>", unsafe_allow_html=True)
...
with col_rec:
    ...
    res_r = st.radio("結果", ["G (得点)", "O (枠外)", "Save", "TF (ミス)", "RTF (ファール)"], horizontal=True)
    sit_options = ["7m"] if st.session_state.selected_zone == '9' else ["Set", "FB"]
    sit_r = st.radio("状況", sit_options, horizontal=True)
    if st.button("記録を確定", use_container_width=True, type="primary"):
        if st.session_state.selected_zone != "未選択" and p_num_r != "未登録":
            ...
            st.session_state.logs.append({"id": st.session_state.log_id_counter, "時間": current_time_str, "チーム": team_rec, "No.": p_num_r, "位置": st.session_state.selected_zone, "結果": res_r.split(" ")[0], "状況": sit_r, "ピリオド": st.session_state.half, "vs_gk": target_gk})
            st.session_state.log_id_counter += 1; st.toast(f"記録完了！ (vs GK.{target_gk})", icon="✅"); time.sleep(0.4); st.rerun()
...
def calc_goals(t, p=None): return sum(1 for l in st.session_state.logs if l["チーム"] == t and l["結果"] == "G" and (p is None or l["ピリオド"] == p))
a_tot, o_tot = calc_goals("味方"), calc_goals("相手"); a_1, o_1, a_2, o_2 = calc_goals("味方", "前半"), calc_goals("相手", "前半"), calc_goals("味方", "後半"), calc_goals("相手", "後半")
st.markdown(f"""
<div class="score-board-container">
    <div class="team-side-a"><div class="team-name-a">{ally_name_in}</div><div class="score-large">{a_tot}</div></div>
    <div class="mid-divider-box">
        <div style="font-size: 11px; color: #64748b;">前半</div><div style="font-size: 18px; font-weight: bold;">{a_1} - {o_1}</div>
        <div style="font-size: 11px; color: #64748b; margin-top:5px;">後半</div><div style="font-size: 18px; font-weight: bold;">{a_2} - {o_2}</div>
    </div>
    <div class="team-side-o"><div class="score-large-opp">{o_tot}</div><div class="team-name-o">{opp_name_in}</div></div>
</div>
""", unsafe_allow_html=True)

a_res = get_stats_logic(st.session_state.logs, "味方", st.session_state.logs); o_res = get_stats_logic(st.session_state.logs, "相手", st.session_state.logs)
stat_items = [("攻撃成功率", "atk_suc"), ("シュート成功率", "sht_suc"), ("FB成功率", "fb_suc"), ("FBシュート成功率", "fb_sht_suc"), ("7m回数", "m7_cnt"), ("7mシュート成功率", "m7_sht_suc"), ("TF回数", "tf"), ("RTF回数", "rtf"), ("シュートセーブ率", "sht_sav"), ("FBセーブ率", "fb_sav"), ("7mセーブ率", "m7_sav")]
for label, key in stat_items:
    ...
    ad = f"{int(av)}" if "回数" in label or "cnt" in key or "tf" in key or "rtf" in key else f"{av:.1f}%"
    od = f"{int(ov)}" if "回数" in label or "cnt" in key or "tf" in key or "rtf" in key else f"{ov:.1f}%"
...
def draw_map(ax, t_name):
    draw_court_base(ax, engine); logs = [l for l in st.session_state.logs if l["チーム"] == t_name and l["位置"] != "9"]
    total = max(1, len(logs)); cmap = plt.cm.Blues if t_name == "味方" else plt.cm.Reds; norm = BoundaryNorm(np.arange(0, 1.2, 0.1), cmap.N)
    for zid, pos in ZONE_LABELS.items():
        if zid == "9": continue
        z_logs = [l for l in logs if l["位置"] == zid]; p = np.array(engine.get_poly(zid)); share = len(z_logs) / total
        ax.fill(p[:,0], p[:,1], color=cmap(norm(share)) if len(z_logs) > 0 else "#fdf2e9", alpha=0.8 if len(z_logs) > 0 else 0.3, zorder=1)
        ax.plot(list(p[:,0])+[p[0,0]], list(p[:,1])+[p[0,1]], color="gray", linewidth=0.8, linestyle=":", zorder=2)
        z_sh = [l for l in z_logs if l["結果"] in ["G", "O", "Save"]]
        if z_sh:
            rate = sum(1 for s in z_sh if s["結果"]=="G") / len(z_sh)
            txt = ax.text(pos[0], pos[1], f"{rate*100:.0f}", ha='center', va='center', fontsize=10, fontweight='bold', zorder=5); txt.set_path_effects([pe.withStroke(linewidth=2, foreground="white")])
with c_map1: f1, a1 = plt.subplots(figsize=(5, 4)); draw_map(a1, "味方"); st.pyplot(f1)
with c_map2: f2, a2 = plt.subplots(figsize=(5, 4)); draw_map(a2, "相手"); st.pyplot(f2)
...
def draw_card(label, team, color, p_list):
    ...
    bg = "rgba(30, 58, 138, 0.05)" if team == "味方" else "rgba(153, 27, 27, 0.05)"
...
with ci1: draw_card(sel_a_label, "味方", "#1e3a8a", st.session_state.ally_players)
with ci2: draw_card(sel_o_label, "相手", "#991b1b", st.session_state.opp_players)
...
def disp_log(label, color):
    t = "味方" if label == ally_name_in else "相手"
    st.markdown(f"<h3 style='color: {color}; text-align: center; border-bottom: 2px solid {color};'>{label}</h3>", unsafe_allow_html=True)
    for p in ["前半", "後半"]:
        pl = [l for l in st.session_state.logs if l["チーム"] == t and l["ピリオド"] == p]
        if pl:
            df = pd.DataFrame(pl); df["位置"] = df["位置"].apply(lambda x: "7m" if x == "9" else x)
            st.data_editor(df, column_order=("時間", "状況", "位置", "No.", "結果", "vs_gk"), hide_index=True, use_container_width=True, key=f"edit_{t}_{p}")
